...
def choose_spec(wman: int) -> Spec:
    ...
    return Spec(wman, n, xf, xw, wt_bits(wman), zf, zw, kinv, 1 << tsa_bits(wman), lut, xf, const2pi)
...
def _emit_consts(s: Spec) -> str:
    ...
    w(f"""
        ...
        output wire        [{cwb - 1:3}:0] const2pi    // round(2*pi * 2**XF), CWB = XF + 3 bits
    """)
    w.pop()
    w(");")
    w.push()
    ...
    w(f"localparam signed [WX-1:0] KINV = {s.xw}'sd{s.kinv};   // round(1/gain * 2**XF)")
    w("")
