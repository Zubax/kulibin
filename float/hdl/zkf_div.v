...
///
/// Register stages: 2+STAGE_INPUT+((WMAN+2+((WMAN+2)%2))/2)+STAGE_OUTPUT end-to-end.
///
...
module zkf_div #(
    ...
    _zkf_pack_delay#(.W(1), .STAGE_OUTPUT(STAGE_OUTPUT)) u_pack_delay(.clk(clk), .rst(1'b0), .x(core_div0), .y(div0));
endmodule
