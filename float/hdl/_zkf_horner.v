...
///
/// STAGE_PRODUCT (0, 1, or 2, with 0 and 1 behaving like zkf_mul/zkf_fma; default 0) splits the multiply for timing
/// closure, costing one extra register stage per degree per level of split:
///
...
///
///   STAGE_PRODUCT=2: 4 register stages per degree. The multiply is a 3x3 product grid of third-width operands;
///                    the 9 sub-products are registered, then summed row-by-acc-chunk into 3 row sums (registered),
///                    then those 3 are combined into the full product (registered). For very wide operands this lets
///                    each sub-product map to a single small multiplier whose output register packs into the DSP,
///                    and keeps every adder in the sum tree shallow. Requires RW >= 3, ACCW >= 3.
///
...
module _zkf_horner #(
    ...
    parameter integer STAGE_PRODUCT = 0   // 0: single mul (2 stages/deg); 1: 2x2 split (3); 2: 3x3 split + row sum (4)
) (
    input  wire                    clk,
    input  wire                    rst,
    input  wire                    in_valid,
    ...
    // verilator coverage_off
    generate
        if ((STAGE_PRODUCT == 1) && (RW < 2)) begin : g_invalid_split2
            _zkf_invalid_stage_product_needs_rw2 u_invalid();
        end
        if ((STAGE_PRODUCT == 2) && ((RW < 3) || (ACCW < 3))) begin : g_invalid_split3
    ...
    // Third-width operand pieces for the STAGE_PRODUCT=2 grid. The two low chunks are unsigned; the top chunk carries
        ...
        for (s = 0; s < D; s = s + 1) begin : g_step
            ...
            if (STAGE_PRODUCT == 0) begin : g_single
                ...
                always @(posedge clk) begin
                    ...
                    m_prod <= a_acc[s] * $signed({1'b0, a_w[s]});
                    m_co   <= a_co[s][COW-1:0];
                    m_w    <= a_w[s];
                    m_sb   <= a_sb[s];
                end
            ...
            end else if (STAGE_PRODUCT == 1) begin : g_split
                // -- Multiply stage: 2x2 grid of half-width products, registered. --
                wire        [WALO-1:0]     acc_lo = a_acc[s][WALO-1:0];
                wire signed [WAHI-1:0]     acc_hi = a_acc[s][ACCW-1:WALO];
                wire        [WWLO-1:0]     w_lo   = a_w[s][WWLO-1:0];
                wire        [WWHI-1:0]     w_hi   = a_w[s][RW-1:WWLO];
                ...
                // -- STAGE_PRODUCT=2: 3x3 grid of third-width sub-products, registered (single-DSP-friendly at very
                ...
                wire        [WA0-1:0] a0 = a_acc[s][WA0-1:0];
                wire        [WA1-1:0] a1 = a_acc[s][WA0+WA1-1:WA0];
                wire signed [WA2-1:0] a2 = a_acc[s][ACCW-1:WA0+WA1];
                wire        [WW0-1:0] b0 = a_w[s][WW0-1:0];
                wire        [WW1-1:0] b1 = a_w[s][WW0+WW1-1:WW0];
                wire        [WW2-1:0] b2 = a_w[s][RW-1:WW0+WW1];
