"""Host side of the serial link to the max5715 bench harness running on the Arty S7.

The framing is defined in ../fpga/arty_s7_max5715_top.v. Every command is answered by exactly one byte, so the
...
CHANNELS = 4
CHANNEL_NAMES = ("OUTA", "OUTB", "OUTC", "OUTD")
...
class Max5715Link:
    """Commands the four DAC channels and the reference selection over the board's USB-UART."""
    ...
    def __enter__(self) -> "Max5715Link":
        return self
    ...
    def status(self) -> dict:
        """Reads the harness telltales, including how many frames the DAC has actually executed.

        The count comes from the device's own RDY pin, which it pulls low on the 24th falling edge of every
        frame it executes. It is therefore evidence from the DAC itself, independent of anything measured on
        the analog side, and it separates "the DAC never saw our frames" from "the outputs are not moving".
        """
        self.ser.reset_input_buffer()
        ...
        flags = reply[2]
        return {
            "frames_executed": (reply[0] << 8) | reply[1],
    ...
    def set_codes(self, codes) -> None:
        """Sets all four DAC codes and updates every output simultaneously."""
        codes = list(codes)
    ...
    def set_signed_mask(self, mask: int) -> None:
        ...
        if not 0 <= mask <= 0x0F:
            raise ValueError(f"mask {mask} does not fit four channels")
        payload = bytearray([CMD_SGN, mask])
        payload.append(_checksum(payload))
        self._transact(bytes(payload), RSP_ACK, f"set_signed_mask(0b{mask:04b})")
    ...
    def set_reference(self, ref: int) -> None:
        ...
        time.sleep(0.01)  # Covers the driver's power-up hold-off and the four configuration frames
