...
module iir1_lpf#(
    ...
    /// Output sample width, sign bit included. Set greater than WIN to expose fractional LSbs from the inner state.
    ...
    /// By default, the full precision is available; otherwise, rounding-to-nearest-ties-to-even is used.
    ...
    // max(WIN, WOUT) avoids truncating the {in, zeros} input cast (requires WX >= WIN); the trailing
    // K+1 bounds the steady-state DC residue 2^(K-WX) below 0.5 LSB of the WOUT-precision output.
    localparam W_STATE = (WOUT > WIN) ? WOUT : WIN;
    localparam WX = W_STATE + K + 1;  // Internal representation width to eliminate DC residue at WOUT.

    wire signed [WX-1:0] x = {in, {(WX-WIN){1'b0}}};  // x[n]   in q1.(WX-1)
    ...
    cast_signed_p#(.WIN(WX), .MSB(0), .LSB(WX-WOUT)) y_out (
        .clk(clk),
        .rst(rst),
